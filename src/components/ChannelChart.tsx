  ...
  return (
    <div className="h-72 w-full">
      <ResponsiveContainer width="100%" height="100%">
        <PieChart>
          <Pie data={data} dataKey="applicants" nameKey="channel" cx="50%" cy="50%" outerRadius={80}>
            {data.map((entry, i) => (
          ...
          <Tooltip
            formatter={(value, _name, props) => [
              `${value}件（${total > 0 ? Math.round((Number(value) / total) * 100) : 0}%）`,
          ...
          <Legend
            ...
            formatter={(value) => {
              const stat = data.find((d) => d.channel === value);
              const pct = stat && total > 0 ? Math.round((stat.applicants / total) * 100) : 0;
              return `${value} ${pct}%`;
            }}
          />
        </PieChart>
